...
function parseEspnEvent(event) {
  ...
  return {
    ...
    venue,
    city,
    // First comma-separated token of the city ("East Rutherford, New Jersey" -> "east rutherford"),
    // used to match our placeholder knockout slots to ESPN events by date + city.
    // City is preferred over stadium name because stadium naming rights change
    // (e.g. Estadio Azteca -> Estadio Banorte) but the city does not.
    cityKey: normalizeKey(city.split(',')[0]),
  };
}
...
function indexEspnEvents(events) {
  ...
  for (const ev of events) {
    ...
    if (ev.dateKey && ev.cityKey) {
      const key = `${ev.dateKey}|${ev.cityKey}`;
      if (!byDateCity.has(key)) byDateCity.set(key, []);
      byDateCity.get(key).push(ev);
    }
  }
  ...
  for (const list of byDateCity.values()) list.sort((a, b) => a.kickoffMs - b.kickoffMs);

  return { byPairDate, byPair, byDateCity };
}
...
function pickEspnEvent(ourMatch, index) {
  ...
  // Real-team path: match by team pair (and ideally date).
  ...
  // Placeholder path: our teams are things like "Winner Match 76". Fall back
  // to matching by date + city, then nearest kickoff time. The knockout
  // schedule is fixed (one match per slot), so this is unambiguous.
  const ourCityKey = normalizeKey((ourMatch.city || '').split(',')[0]);
  if (!dateKey || !ourCityKey || ourMs == null) return null;

  const candidates = index.byDateCity.get(`${dateKey}|${ourCityKey}`) || [];
  if (!candidates.length) return null;
...
// Resolve team names for a placeholder match from the matched ESPN event.
// Returns { teamA, teamB } when our teams are placeholders, otherwise null.
// ESPN's home team maps to teamA, away to teamB.
function teamsForMatch(ourMatch, espnEvent) {
  const teamA = canonicalTeam(ourMatch.teamA);
  const teamB = canonicalTeam(ourMatch.teamB);
  if (isRealTeam(teamA) && isRealTeam(teamB)) return null;
  if (!espnEvent.homeTeam || !espnEvent.awayTeam) return null;
  return { teamA: espnEvent.homeTeam, teamB: espnEvent.awayTeam };
}

function buildPatch(ourMatch, espnEvent) {
  const teams = teamsForMatch(ourMatch, espnEvent);

  // Effective team names after resolution, so we can map ESPN scores even
  // on the same run that first resolves the placeholder.
  const teamA = teams ? teams.teamA : canonicalTeam(ourMatch.teamA);
  const teamB = teams ? teams.teamB : canonicalTeam(ourMatch.teamB);
  ...
  return {
    ...
    scoreA: espnEvent.status === 'upcoming' ? null : scoreA,
    scoreB: espnEvent.status === 'upcoming' ? null : scoreB,
    ...(teams || {}),
  };
}
...
function shouldApply(current, next) {
  ...
  // Team resolution (placeholder -> real names) always applies.
  if (next.teamA != null && next.teamA !== current.teamA) return true;
  if (next.teamB != null && next.teamB !== current.teamB) return true;
...
export function buildScoreUpdates(ourMatches, espnEvents) {
  ...
  for (const match of ourMatches) {
    ...
    const patch = buildPatch(match, espn);
...
export function applyScoreUpdates(matches, updates) {
  ...
  for (const patch of updates) {
    ...
    if (patch.teamA != null && patch.teamA !== match.teamA) match.teamA = patch.teamA;
    if (patch.teamB != null && patch.teamB !== match.teamB) match.teamB = patch.teamB;
    changed += 1;
  }
